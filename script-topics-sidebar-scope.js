// Topics page: sidebar-driven scope.
// The Company / Department selectors in the sidebar drive which topics render.
...
function deptsForCompany(companyId) {
    return SIDEBAR_DEPARTMENTS.filter(function(d) { return d.companyId === companyId; });
}

function readSidebarScope() {
    try {
...
function writeSidebarScope(companyId, departmentId) {
    try {
...
function applyTopicsScope(companyId, departmentId) {
    writeSidebarScope(companyId, departmentId);
    ...
    ['topicsContent', 'topicsTable', 'topicsListToolbar'].forEach(function(id) {
    ...
    if (!ready) {
        var sc = document.getElementById('scopeCompany');
        var sd = document.getElementById('scopeDept');
        var si = document.getElementById('scopeInput');
        if (sc) sc.value = '';
        if (sd) sd.value = '';
        if (si) si.value = '';
        var countEl = document.getElementById('topicsCount');
        if (countEl) {
            countEl.innerHTML = '';
            countEl.hidden = true;
        }
        var scopeLabelEmpty = document.getElementById('topicsScopeLabel');
        if (scopeLabelEmpty) scopeLabelEmpty.textContent = '';
        return;
    }

    var company = SIDEBAR_COMPANIES.find(function(c) { return c.id === companyId; });
    if (!company) return;
    var department = hasDept
        ? SIDEBAR_DEPARTMENTS.find(function(d) { return d.id === departmentId; })
        : null;
    if (hasDept && !department) return;

    var companyKey = company.name.toLowerCase();
    var deptName = department ? department.name : '';
    var sc2 = document.getElementById('scopeCompany');
    var sd2 = document.getElementById('scopeDept');
    var si2 = document.getElementById('scopeInput');
    if (sc2) sc2.value = companyKey;
    if (sd2) sd2.value = deptName;
    if (si2) si2.value = department ? (deptName + ' — ' + company.name) : company.name;

    var scopeLabel = document.getElementById('topicsScopeLabel');
    if (scopeLabel) {
    ...
    if (typeof onScopeReady === 'function') {
        onScopeReady(companyKey, deptName);
    }
}

$(function() {
    var stored = readSidebarScope();
    applyTopicsScope(stored.companyId || null, stored.departmentId || null);

    document.addEventListener('gameon:scope-change', function(e) {
        var d = (e && e.detail) || {};
        applyTopicsScope(d.companyId == null ? null : d.companyId,
